...
- **判明した事実**:
    - **初期化エラー**: 解消。`Langfuse` クラスによる明示的初期化に成功。
    - **リージョン確認**: 成功。画像により Host(SSM) と実体(US) の一致を確認済み。
    - **404 エラー**: プロンプト取得時に発生。ラベル・リージョン以外の要因（名称の不一致等）を疑う。
- **次への引継ぎ**: `docs/調査中.md` に詳細を整理済み。新規チャットにて「ラベルなし取得テスト」から開始。
